        ...
        <CreateFormCard onSubmit={props.createTrip}>
            ...
            <select name="planet" value={props.form.planet} onChange={props.onChange} required >
                ...
                {listaPlanetas.map((planeta) => {
                    return <option key={planeta}>{planeta}</option>
                })}
            </select>

            <h3>Nome da Viagem</h3>
            <input name="name" type="text" value={props.form.name} onChange={props.onChange} pattern="{5,}" required />
            
            <h3>Descrição</h3>
            <input name="description" type="text" value={props.form.description} onChange={props.onChange} pattern="{30,}" required />
